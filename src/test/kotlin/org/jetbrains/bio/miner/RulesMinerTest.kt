...
import org.jetbrains.bio.rule.Distribution
import org.jetbrains.bio.rule.Distribution.Companion.kullbackLeibler
import org.jetbrains.bio.rule.EmpiricalDistribution
...
class RulesMinerTest : TestCase() {
    ...
    private fun <T> FishboneMiner.Node<T>.structure(database: List<T>,
        ...
        while (node != null) {
            result.add("${node.element.name()}(" +
                    ...
                    (if (logKL) ",${String.format("%.2f", kullbackLeibler(empirical, independent.learn(node.rule)) / kl)}" else "") +
                    ")")
            node = node.parent
        }
    ...
    private fun testKL(function: KProperty1<Rule<Int>, Double>) {
        ...
        listOf(100, 1000, 5000).forEach { size ->
            ...
            for (c in 1..5) {
                val condition = optimizeWithProbes(target, database, predicates, maxComplexity = c, function = function).element
    ...
    fun testOptimizeConvictionThresholds() {
        val predicates = (0..5).map { RangePredicate(2.0.pow(it.toDouble()).toInt(), 2.0.pow(it.toDouble() + 1).toInt()) }
        ...
        val r05 = optimizeWithProbes(RangePredicate(0, 80), database, predicates,
        ...
        assertEquals("[32;64)(6.65,0.76),[16;32)(9.98,0.57),[8;16)(11.64,0.43),[4;8)(12.48,0.35)",
                r05.structure(database, function = Rule<Int>::conviction))

        val r1 = optimizeWithProbes(RangePredicate(0, 80), database, predicates,
    ...
    fun testOptimizeLoeThresholds() {
        ...
        assertEquals("[16;32) OR [1;2) OR [2;4) OR [32;64) OR [4;8) OR [8;16)", r0.rule.conditionPredicate.name())
        assertEquals("[32;64)(2.66,0.79),[16;32)(2.99,0.62),[8;16)(3.11,0.50),[4;8)(3.16,0.42),[2;4)(3.19,0.38),[1;2)(3.20,0.35)",
                r0.structure(database, function = Rule<Int>::loe))

        val r01 = optimizeWithProbes(RangePredicate(0, 80), database, predicates,
                function = Rule<Int>::loe, functionDelta = 0.1, klDelta = -1.0)
        assertEquals("[16;32) OR [32;64) OR [8;16)", r01.rule.conditionPredicate.name())
        assertEquals("[32;64)(2.66,0.72),[16;32)(2.99,0.49),[8;16)(3.11,0.33)",
    ...
    private fun testOptimize(function: KProperty1<Rule<Int>, Double>) {
        ...
        listOf(100, 1000, 5000).forEach { size ->
            ...
            assertEquals(size.toString(), "[20;30) OR [30;40) OR [40;50)",
                    optimizeWithProbes(RangePredicate(20, 50), database, predicates, function = function).rule.conditionPredicate.name())
            assertEquals(size.toString(), "[20;30) OR [30;40) OR [40;50) OR [50;60)",
                    optimizeWithProbes(RangePredicate(20, 60), database, predicates, function = function).rule.conditionPredicate.name())
            assertEquals(size.toString(), "[20;30) OR [30;40) OR [40;50)",
                    optimizeWithProbes(RangePredicate(20, 51), database, predicates, function = function).rule.conditionPredicate.name())
        }
    }
